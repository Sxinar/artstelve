<script>
  ...
  import { buildBangUrl, resolveBang } from "$lib/search-preferences.js";
  ...
  function performSearchNavigation() {
    ...
    if (bang.startsWith("!") && bangCommand) {
      ...
      const url = buildBangUrl(bangCommand, query);
      if ($bangsOpenNewTab) {
...
<svelte:window onpointerdown={clickOutsideSuggestions} />
...
>
  ...
  <div class="search-container">
    <div class="search-box" onclick={(e) => e.stopPropagation()} onpointerdown={(e) => e.stopPropagation()}>
...
<style>
  ...
  @media (max-width: 768px) {
    ...
    .logo-container {
      margin: clamp(2rem, 12vh, 5rem) 0 1.5rem;
      gap: 0.65rem;
    }
