<script>
    ...
    import { buildBangUrl, normaliseDomain, resolveBang } from "$lib/search-preferences.js";
    ...
    function performSearch(query, skipSpelling = false) {
        ...
        const bangCommand = resolveBang(bang, $customBangs);
        if (bang.startsWith("!") && bangCommand) {
            const searchQ = parts.slice(1).join(" ");
            const url = buildBangUrl(bangCommand, searchQ);
            if ($bangsOpenNewTab) {
    ...
    function blockSite(siteUrl) {
        ...
        if (domain) {
            siteRules.update((rules) => [...rules.filter((rule) => rule.domain !== domain), { domain, action: "block", createdAt: new Date().toISOString() }]);
            blockedSites.update((sites) => sites.includes(domain) ? sites : [...sites, domain]);
            openMenuUrl = null;
        }
    }

    function boostSite(siteUrl) {
        const domain = getDomain(siteUrl);
        if (domain) {
            siteRules.update((rules) => [...rules.filter((rule) => rule.domain !== domain), { domain, action: "boost", createdAt: new Date().toISOString() }]);
    ...
    let filteredResults = $derived.by(() => {
        if (activeSearchType !== "web") return $searchResults;
        const rules = new Map($siteRules.map((rule) => [rule.domain, rule.action]));
        const legacyBlocked = new Set($blockedSites);
        return $searchResults
            .filter((result) => {
                const domain = normaliseDomain(result.url);
                return rules.get(domain) !== "block" && !legacyBlocked.has(domain);
            })
            .map((result, index) => ({ result, index, boosted: rules.get(normaliseDomain(result.url)) === "boost" }))
            .sort((a, b) => Number(b.boosted) - Number(a.boosted) || a.index - b.index)
            .map(({ result }) => result);
    });
    ...
    function handleInput(event) {
        ...
        if (isBang) {
            ...
            const availableBangs = { ...BANG_COMMANDS, ...Object.fromEntries($customBangs.map((bang) => [bang.trigger, bang])) };
            suggestions = Object.keys(availableBangs)
                .filter((bang) => bang.startsWith(qLower))
                .map((bang) => ({
                    text: bang,
                    description: availableBangs[bang].name,
                    isBang: true,
                }));
...
<div class="search-results-page">
    ...
    <div class="search-main-content" class:images-mode={activeSearchType === 'images'}>
        <main class="results-container" aria-live="polite">
            {#if isLoading}
                <div class="search-skeleton" class:image-skeleton={activeSearchType === "images"} aria-label="Sonuçlar yükleniyor" aria-busy="true" in:fade={{ duration: 200 }}>
                    {#each Array(activeSearchType === "images" ? 12 : 6) as _}
                ...
                <div class="search-results-list">
                    ...
                    {#if activeSearchType === "web"}
                        {#if filteredResults.length > 0}
                            <div class="results-list web-results view-{$searchResultsView}">
                                ...
                                {#each filteredResults as result (result.url)}
                                    <div class="result-item-card">
                                            ...
                                            <div class="result-menu-container">
                                                ...
                                                {#if openMenuUrl === result.url}
                                                    ...
                                                    >
                                                        ...
                                                        <button onclick={() => boostSite(result.url)}>
                                                            <i class="fas fa-arrow-up"></i> Bu siteyi öne çıkar
                                                        </button>
                                                        <button
                                                            onclick={() => {
                                        ...
                                        <h3 class="result-title">
                                            <a
                                                href={result.url}
                                                target={$searchResultsOpenNewTab ? "_blank" : "_self"}
            ...
            <aside class="infobox-container">
                ...
                <div class="infobox-bottom-row">
                    {#if $enableRelatedNews && relatedNewsItems.length > 0}
                        <div class="widget-card news-widget mobile-related-widget">
                    ...
                    {#if $enableRelatedSearches && relatedSearchItems.length > 0}
                        <div class="widget-card searches-widget mobile-related-widget">
...
<style>
    ...
    .search-skeleton.image-skeleton { grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); }
    .skeleton-card { padding: 1.15rem; min-height: 118px; display: grid; align-content: start; gap: .7rem; background: var(--card-background); border: 1px solid var(--border-color); border-radius: var(--radius-base); }
    .image-skeleton .skeleton-card { min-height: 180px; }
    .skeleton-line { border-radius: 999px; background: linear-gradient(90deg, var(--hover-background) 25%, color-mix(in srgb, var(--hover-background) 45%, var(--card-background)) 50%, var(--hover-background) 75%); background-size: 240% 100%; animation: skeleton-shimmer 1.25s ease-in-out infinite; }
    .skeleton-domain { width: 28%; height: .7rem; }
    .skeleton-title { width: 72%; height: 1.15rem; }
    .skeleton-copy { width: 100%; height: .8rem; }
    .skeleton-copy.short { width: 62%; }
    .image-skeleton .skeleton-domain, .image-skeleton .skeleton-title, .image-skeleton .skeleton-copy { display: none; }
    .image-skeleton .skeleton-card { background: linear-gradient(90deg, var(--hover-background) 25%, color-mix(in srgb, var(--hover-background) 45%, var(--card-background)) 50%, var(--hover-background) 75%); background-size: 240% 100%; animation: skeleton-shimmer 1.25s ease-in-out infinite; }
    .search-skeleton > p { display: none; }
    @keyframes skeleton-shimmer { to { background-position: -240% 0; } }

    /* Result layouts use a single structural class; theme variables carry the visual system. */
    .web-results.view-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1rem; }
    .web-results.view-grid .result-item-card { height: 100%; margin: 0; }
    .web-results.view-list { display: grid; gap: .25rem; }
    .web-results.view-list .result-item-card { padding: .8rem 0; margin: 0; background: transparent; border: 0; border-bottom: 1px solid var(--border-color); border-radius: 0; box-shadow: none; }
    .web-results.view-list .result-item-card:hover { transform: none; box-shadow: none; background: var(--hover-background); }
    .web-results.view-list .result-description { display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical; overflow: hidden; }
    .web-results.view-magazine { display: grid; gap: 1.25rem; }
    .web-results.view-magazine .result-item-card { position: relative; min-height: 130px; padding: 1.25rem 1.25rem 1.25rem 2rem; overflow: hidden; }
    .web-results.view-magazine .result-item-card::after { content: ''; position: absolute; inset: 0 auto 0 0; width: 6px; background: var(--primary-color); }
    .web-results.view-magazine .result-title { font-size: 1.3rem; }
    .web-results.view-magazine .result-description { font-size: 1rem; line-height: 1.7; }
    @media (max-width: 640px) { .web-results.view-grid { grid-template-columns: 1fr; } }
    ...
    .web-results .result-item-card:hover {
        border-color: color-mix(in srgb, var(--primary-color) 45%, var(--border-color));
        box-shadow: 0 8px 22px rgba(24, 39, 75, 0.1);
    }
    ...
    @media (max-width: 768px) {
        ...
        .mobile-related-widget {
            display: none !important;
        }

        .infobox-bottom-row:has(.mobile-related-widget) {
            display: none;
        }

        .web-results.view-grid,
        .web-results.view-magazine {
            display: grid;
            grid-template-columns: 1fr;
            gap: 0.75rem;
        }
        ...
        .web-results .result-item-card,
        .web-results.view-grid .result-item-card,
        .web-results.view-magazine .result-item-card {
            min-height: 0;
            padding: 0.95rem;
            border-radius: 12px;
        ...
        .web-results .result-title {
            font-size: 1rem;
            line-height: 1.35;
        }

        .web-results .result-description {
            display: -webkit-box;
            -webkit-line-clamp: 3;
            -webkit-box-orient: vertical;
            overflow: hidden;
    ...
    @media (max-width: 768px) {
        ...
        .web-results .result-header {
            min-width: 0;
            gap: 0.4rem;
        }

        .web-results .result-domain {
            min-width: 0;
            overflow: hidden;
            text-overflow: ellipsis;
            white-space: nowrap;
