<script>
    ...
    import { safeSearch } from '$lib/stores.js';
    ...
    function changeSearchType(newType) {
         if (newType !== activeSearchType) {
             ...
             imageSize = imageColor = imageAspect = imageType = imagePalette = '';
...
<div class="search-results-page">
    ...
    <!-- Search Type Tabs -->
     <nav class="search-type-nav" aria-label="Arama türleri">
         <div class="search-type-nav-inner">
             ...
             <button class:active={activeSearchType === 'news'} on:click={() => changeSearchType('news')}>
                 <i class="fas fa-newspaper" aria-hidden="true"></i> {$t('news')}
             </button>
    ...
    {#if activeSearchType === 'images'}
        <div class="image-filters" role="region" aria-label="Görsel filtreleri">
            <div class="select-wrapper">
                <select bind:value={imageSize} on:change={() => { offset = 0; handleSearchSubmit('images'); }} aria-label="Boyut">
            ...
            <div class="select-wrapper">
                <select bind:value={imageColor} on:change={() => { offset = 0; handleSearchSubmit('images'); }} aria-label="Renk">
            ...
            <div class="select-wrapper">
                <select bind:value={imageAspect} on:change={() => { offset = 0; handleSearchSubmit('images'); }} aria-label="En/Boy Oranı">
            ...
            <div class="select-wrapper">
                <select bind:value={imageType} on:change={() => { offset = 0; handleSearchSubmit('images'); }} aria-label="Tür">
            ...
            <div class="select-wrapper">
                <select bind:value={imagePalette} on:change={() => { offset = 0; handleSearchSubmit('images'); }} aria-label="Palet">
    ...
    {#if activeSearchType === 'news'}
        <div class="news-filters" role="region" aria-label="Haber filtreleri">
            <input class="news-filter-input" type="text" placeholder="Kaynak (domain veya isim)" bind:value={newsSource} on:change={() => { offset = 0; handleSearchSubmit('news'); }} />
            <input class="news-filter-input" type="date" bind:value={newsStartDate} on:change={() => { offset = 0; handleSearchSubmit('news'); }} />
            <input class="news-filter-input" type="date" bind:value={newsEndDate} on:change={() => { offset = 0; handleSearchSubmit('news'); }} />
        </div>
    {/if}
    {#if activeSearchType === 'images'}
        <div class="image-filters" role="region" aria-label="Görsel filtreleri">
            <div class="select-wrapper">
                <select bind:value={imageSize} on:change={() => handleSearchSubmit('images')} aria-label="Boyut">
                    <option value="">Boyut: Tümü</option>
                    <option value="small">Küçük</option>
                    <option value="medium">Orta</option>
                    <option value="large">Büyük</option>
            ...
            <div class="select-wrapper">
                <select bind:value={imageColor} on:change={() => handleSearchSubmit('images')} aria-label="Renk">
                    <option value="">Renk: Tümü</option>
                    <option value="color">Renkli</option>
                    <option value="monochrome">Siyah-Beyaz</option>
                    <option value="red">Kırmızı</option>
            ...
            <div class="select-wrapper">
                <select bind:value={imageAspect} on:change={() => handleSearchSubmit('images')} aria-label="En/Boy Oranı">
                    <option value="">Oran: Tümü</option>
                    <option value="square">Kare</option>
                    <option value="wide">Geniş</option>
                    <option value="tall">Yüksek</option>
             ...
             {:else if activeSearchType === 'images'}
                 {#if $searchResults.length > 0}
                    ...
                    <div class="load-more-wrapper">
                        <button class="button button-secondary" on:click={loadMore}>Daha fazla yükle</button>
                    </div>
                {:else if searchQuery}
                     <div class="no-results">
                    ...
                    <div class="results-list news-results">
                        {#each $searchResults as result (result.url)}
                            <div class="result-item-card news-item-card">
                                <h3 class="result-title"><a href={result.url} target="_blank" rel="noopener noreferrer">{result.title}</a></h3>
                                <div class="news-meta">
                                    {#if result.source}<span class="news-source">{result.source}</span>{/if}
                                    {#if result.age}<span class="news-age">{result.age}</span>{/if}
                                </div>
                                {#if result.thumbnail}
                                    <img src={result.thumbnail} alt="" class="news-thumbnail" loading="lazy" on:error={(e) => { e.target.style.display='none'; }}/>
                                {/if}
                                {#if result.description}<p class="result-description">{result.description}</p>{/if}
                            </div>
                        {/each}
                    </div>
                    <p class="results-count">{$searchResults.length} haber bulundu.</p>
                    <div class="load-more-wrapper">
                        <button class="button button-secondary" on:click={loadMore}>Daha fazla yükle</button>
                    </div>
                {:else if searchQuery}
                    <div class="no-results"><p>'{searchQuery}' için haber bulunamadı.</p></div>
                {/if}
...
<style>
    ...
    .results-list { gap: 1.1rem; }
    .result-title { letter-spacing: 0.1px; }
    .result-description { color: var(--text-color-secondary); }

    /* Load more area */
    .load-more-wrapper { display: flex; justify-content: center; margin-top: 0.8rem; }
    ...
    .result-item-card::before {
        ...
        right: 0;
        height: 3px;
        background: linear-gradient(90deg, var(--primary-color), rgba(var(--primary-color-rgb), 0.3));
        opacity: 0;
    ...
    .news-item-card .result-title { margin-bottom: 0.4rem; }
    .news-item-card .result-description { margin-top: 0.3rem; }
    .news-meta {
        display: flex;
        gap: 0.8rem;
        align-items: center;
