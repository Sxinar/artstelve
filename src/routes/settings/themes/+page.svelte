<script lang="js">
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.success) {
                    ...
                    // Ana sayfa temalarını tema kategorisinden ayır.
                    const _homeThemes = allThemes.filter((p) => {
                        const type = (p.category || "").toLowerCase();
                        return type === "home" || type === "ana_sayfa";
                    });

                    homeThemes.set(_homeThemes);

                    // Logolar API'den doğrudan logos dizisi olarak geliyor
    ...
    async function applyWorkshopItem(item, type) {
        ...
        if (type === "theme") {
            if (item.category === "home") {
                ...
                try {
                    console.log(
                        "[Themes] Starting theme application for:",
                        item.name,
                        "ID:",
                        item.id,
                    );
                    ...
                    const response = await fetch(
                        `/api/workshop/css?url=${encodeURIComponent(item.download_url)}`,
                    );
                    if (!response.ok) {
                    ...
                    console.log(
                        "[Themes] CSS content length:",
                        cssContent.length,
                    );
                    console.log(
                        "[Themes] CSS content preview:",
                    ...
                    try {
                        ...
                        const themeClassMatch =
                            cssContent.match(/body\.(\w+)\s*\{/);
                        console.log(
                            "[Themes] Theme class match result:",
                            themeClassMatch,
                        );
                        themeClass = themeClassMatch
                            ? themeClassMatch[1]
                            : null;
                        console.log(
                            "[Themes] Extracted theme class:",
                            themeClass,
                        );
                    } catch (error) {
                        console.error(
                            "[Themes] Error extracting theme class:",
                            error,
                        );
                        console.error(
                            "[Themes] Error details:",
                            error.message,
                            error.stack,
                        );
                    }
                    ...
                    const existingStyles = document.querySelectorAll(
                        'style[id^="workshop-theme-"]',
                    );
                    console.log(
                        "[Themes] Found existing styles:",
                        existingStyles.length,
                    );

                    existingStyles.forEach((style, index) => {
                        console.log(
                            `[Themes] Processing style ${index}:`,
                            style.id,
                        );
                        try {
                            if (style.textContent) {
                                console.log(
                                    `[Themes] Style ${index} content length:`,
                                    style.textContent.length,
                                );
                                const classMatch =
                                    style.textContent.match(/body\.(\w+)\s*\{/);
                                console.log(
                                    `[Themes] Style ${index} class match:`,
                                    classMatch,
                                );
                                if (classMatch && classMatch[1]) {
                                    console.log(
                                        `[Themes] Removing class:`,
                                        classMatch[1],
                                    );
                                ...
                                console.log(
                                    `[Themes] Style ${index} has no textContent`,
                                );
                            }
                        } catch (error) {
                            console.error(
                                "[Themes] Error removing theme class:",
                                error,
                            );
                            console.error(
                                "[Themes] Error details:",
                    ...
                    console.log(
                        "[Themes] Creating style element with ID:",
                        styleId,
                    );
                    ...
                    if (themeClass) {
                        console.log(
                            "[Themes] Applying body class:",
                            themeClass,
                        );
                        document.body.classList.add(themeClass);
                        console.log(
                            "[Themes] Body classes after adding:",
    ...
    async function installItem(item, type) {
        ...
        try {
            ...
            if (type === "theme" && item.category === "home") {
                ...
                alert(
                    `Workshop temaları geçici olarak devre dışı. Lütfen daha sonra tekrar deneyin.\n\nTema: ${item.name}`,
                );
            } else {
                await applyWorkshopItem(item, type);
            }
            ...
            if (response.ok) {
                ...
                if (data.success) {
                    ...
                    const activeTheme = themes.find(
                        (t) => t.id === activeThemeId && t.category !== "home",
                    );
                    if (activeTheme && activeTheme.download_url) {
                        try {
                            // Use server-side proxy to fetch CSS content
                            const cssResponse = await fetch(
                                `/api/workshop/css?url=${encodeURIComponent(activeTheme.download_url)}`,
                            );
                            if (cssResponse.ok) {
                                ...
                                try {
                                    ...
                                    console.error(
                                        "[Themes] Error extracting theme class during reload:",
                                        error,
                                    );
                                }
                                ...
                                if (themeClass) {
                                    document.body.classList.add(themeClass);
                                    console.log(
                                        `[Themes] Re-applied body class: ${themeClass}`,
                                    );
                                }
                            }
                        } catch (error) {
                            console.error(
                                "Aktif tema yeniden yüklenemedi:",
                                error,
                            );
                        }
                    }
    ...
    function getActiveThemes() {
        ...
        console.log(
            "[Themes] Debug - selectedTheme:",
            $selectedTheme,
            "Type:",
            typeof $selectedTheme,
        );
        console.log(
            "[Themes] Debug - searchHomeDesign:",
            $searchHomeDesign,
            "Type:",
            typeof $searchHomeDesign,
        );
        ...
        if (selectedThemeValue && typeof selectedThemeValue === "string") {
            try {
                const isWorkshopTheme =
                    selectedThemeValue.startsWith("http://") ||
                    selectedThemeValue.startsWith("https://");
                if (isWorkshopTheme) {
                    // Find this theme in workshop themes
                    const workshopTheme = currentThemes.find(
                        (t) => t.download_url === selectedThemeValue,
                    );
                    if (workshopTheme) {
                        activeThemes.push({
                            ...workshopTheme,
                ...
                console.error(
                    "[Themes] Error checking selectedTheme:",
                    error,
                    "Value:",
                    selectedThemeValue,
                );
        ...
        if (
            searchHomeDesignValue &&
            typeof searchHomeDesignValue === "string"
        ) {
            try {
                const isWorkshopHomeTheme =
                    searchHomeDesignValue.startsWith("http://") ||
                    searchHomeDesignValue.startsWith("https://");
                if (isWorkshopHomeTheme) {
                    // Find this theme in workshop themes.
                    const workshopHomeTheme = currentThemes.find(
                        (t) =>
                            t.download_url === searchHomeDesignValue &&
                            (t.category === "home" ||
                                t.category === "ana_sayfa"),
                    );
                    if (workshopHomeTheme) {
                        ...
                        console.log(
                            "[Themes] Added home theme:",
                            workshopHomeTheme.name,
                        );
                    }
                }
            } catch (error) {
                console.error(
                    "[Themes] Error checking searchHomeDesign:",
                    error,
                    "Value:",
                    searchHomeDesignValue,
                );
    ...
    $: {
        ...
        installedGeneralThemes = activeThemes.filter(
            (t) => t.category !== "home",
        );
    ...
    async function uninstallItem(id, type) {
        if (
            !confirm("Bu temanın seçimini kaldırmak istediğinize emin misiniz?")
        )
            return;
                    ...
                    try {
                        ...
                        if (cssContent) {
                            ...
                            if (classMatch && classMatch[1]) {
                                ...
                                console.log(
                                    `[Themes] Removed body class: ${classMatch[1]}`,
                                );
                            }
                        }
                    } catch (error) {
                        console.error(
                            "[Themes] Error extracting theme class during uninstall:",
                            error,
                        );
                    }
                    styleElement.remove();
